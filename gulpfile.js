...
gulp.task('copy:html', () => gulp.src('src/*.html').pipe(cleanhtml()).pipe(gulp.dest('build')));
...
gulp.task('copy', gulp.series('copy:icons', 'copy:locales', 'copy:manifest', 'copy:html', 'copy:scripts'));
...
gulp.task('zip:map', () => {
  ...
  return gulp.src('build/scripts/**/*.map').pipe(zip(mapFileName)).pipe(gulp.dest('dist'));
});
...
gulp.task('zip:dist', () => {
  ...
  return gulp.src(['build/**', '!build/scripts/**/*.map']).pipe(zip(distFileName)).pipe(gulp.dest('dist'));
});

// build ditributable and sourcemaps after other tasks completed
gulp.task('zip', gulp.series('zip:dist', 'zip:map'));

// build ditributable and sourcemaps after other tasks completed
gulp.task('build', gulp.series('copy', 'zip'));
